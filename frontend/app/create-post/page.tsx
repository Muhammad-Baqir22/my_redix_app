  ...
  useEffect(() => {
    ...
    apiFetch<ApiResponse<ApiSubreddit[]>>("/api/subreddit/subs/")
      .then((res) => {
        ...
        const all = res.data ?? [];
        const list = all.filter((s) => s.is_following || s.created_by === userId);
        setSubreddits(list);
    ...
    <div className="min-h-screen" style={{ background: "var(--bg-base)" }}>
      ...
      <div className="flex pt-14">
        ...
        <main className="flex-1 sidebar-ml px-4 py-8">
          <h1 className="text-[var(--text-1)] text-2xl font-bold mt-5 mb-6">Create a post</h1>
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              <label className="text-[var(--text-2)] text-xs font-semibold uppercase tracking-wider mb-2 block">
                Community (optional)
              </label>
              {subreddits.length === 0 ? (
                <p className="text-[var(--text-3)] text-sm">No communities joined yet. Posts go to your personal feed.</p>
              ) : (
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                      selectedSub === ""
                        ...
                        : "border-[var(--border)] text-[var(--text-3)] hover:border-purple-500/30 hover:text-[var(--text-1)]"
                    }`}
                  >
                    My feed
                  </button>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                          active
                            ? "border-purple-500/60 bg-purple-500/15 text-purple-300"
                            : "border-[var(--border)] text-[var(--text-2)] hover:border-purple-500/30 hover:text-[var(--text-1)]"
                        }`}
                      >
            ...
            >
              ...
              <div className="p-4 border-b border-[var(--border)]">
                <input
                  ...
                  maxLength={300}
                  className="w-full bg-transparent text-[var(--text-1)] text-lg font-semibold placeholder-[var(--text-3)] focus:outline-none"
                />
                <div className="flex justify-end mt-1">
                  <span className="text-[var(--text-3)] text-xs">{title.length}/300</span>
                </div>
              </div>
              ...
              <div className="p-4 border-b border-[var(--border)]">
                <textarea
                  ...
                  rows={5}
                  className="w-full bg-transparent text-[var(--text-2)] text-sm placeholder-[var(--text-3)] resize-none focus:outline-none leading-relaxed"
                />
              </div>
              ...
              {(preview || uploading) && (
                <div className="px-4 py-3 border-b border-[var(--border)]">
                  <div className="relative w-full rounded-xl overflow-hidden bg-[var(--bg-hover)]">
                    {uploading && (
              ...
              <div className="flex items-center justify-between px-4 py-3">
                <button
                  ...
                  disabled={uploading}
                  className="flex items-center gap-1.5 text-[var(--text-3)] hover:text-purple-400 text-xs font-medium transition-colors disabled:opacity-50"
                >
                  {uploading ? (
                ...
                <div className="flex gap-2">
                  <Link
                    href="/"
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium text-[var(--text-2)] border border-[var(--border)] hover:bg-[var(--bg-hover)] transition-all"
                  >
                    <X size={14} />
                    Cancel
                  </Link>
