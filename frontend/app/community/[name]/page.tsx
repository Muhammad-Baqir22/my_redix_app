        ...
        <div className="flex pt-14">
          ...
          <main className="flex-1 sidebar-ml flex items-center justify-center px-4">
            <div className="text-center">
              <p className="text-[var(--text-2)] text-lg font-semibold mb-2">Community not found</p>
              <p className="text-[var(--text-3)] text-sm mb-4">r/{name} doesn&apos;t exist yet.</p>
          ...
          <div className="px-4 max-w-2xl mx-auto">
            ...
            <div ref={avatarRef} className="-mt-8 sm:-mt-10 mb-3">
              <div
                ...
                style={{ background: "linear-gradient(135deg, #7c3aed, #6366f1)", borderColor: "var(--bg-base)" }}
              >
            ...
            <div className="flex items-start justify-between gap-2 mb-1">
              <div className="min-w-0 flex-1">
                {loadingInfo ? (
                  <div className="space-y-2 animate-pulse">
                    <div className="h-6 w-36 bg-[var(--bg-hover)] rounded" />
                    <div className="h-3 w-52 bg-[var(--bg-hover)] rounded" />
                  </div>
                ) : community && (
                  <>
                    <h1 className="text-[var(--text-1)] font-bold text-lg sm:text-2xl leading-tight truncate">
                      r/{community.name}
                    </h1>
                    {community.description && (
                      <p className="text-[var(--text-2)] text-xs sm:text-sm mt-0.5 line-clamp-2">
                        {community.description}
                      </p>
                    )}
                    <div className="flex items-center gap-1.5 mt-1.5 text-[var(--text-3)] text-xs">
                      <Users size={11} />
              ...
              {community && (() => {
                ...
                const isAdmin = community.created_by === userId;
                return (
                  <div className="flex items-center gap-1.5 flex-shrink-0 pt-1">
                    {(isAdmin || community.is_following) && (
                      <Link
                        href={`/create-post?community=${community.name}&communityId=${community.id}`}
                        className="flex items-center gap-1 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl text-[var(--text-1)] text-xs font-semibold border border-[var(--border)] hover:bg-[var(--bg-hover)] transition-all"
                      >
                        <PenSquare size={12} />
                    ...
                    {isAdmin ? (
                      <span className="text-xs font-semibold text-purple-400 border border-purple-500/40 px-2.5 py-1.5 rounded-lg">
                        Admin
                      </span>
                    ) : (
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1.5 sm:px-4 sm:py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap ${
                          community.is_following ? "text-[var(--text-2)]" : "text-white"
                        }`}
            ...
            <div className="flex flex-col gap-3 pb-6">
              {loadingPosts && Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="h-28 rounded-2xl animate-pulse" style={{ background: "var(--bg-hover)" }} />
              ))}

              {!loadingPosts && posts.length === 0 && (
                <div className="text-center py-14">
                  <p className="text-[var(--text-3)] text-sm mb-3">No posts yet in this community.</p>
                  {community && (
                    <Link
                      href={`/create-post?community=${community.name}&communityId=${community.id}`}
